  ...
  if($_POST['newpass']==$_POST['cnewpass'])
    {
      $Username=$_SESSION['username'];
        ...
        $Newpass=$_POST['newpass'];
         $sql="SELECT * FROM `department_head` WHERE `Email_ID`='$Username'";
         $result=mysqli_query($connection,$sql);
          if ($result->num_rows > 0) 
          {
            while($row = $result->fetch_assoc()) {
              ...
              if($Oldpass==$pass)
              {
                $sql="UPDATE `department_head` SET `PASSWORD`= '$Newpass' WHERE `Email_ID`='$Username' && `ID`='$id'";
                 echo $sql;
                  $result=mysqli_query($connection,$sql);
                if ($result) 
                {
...
function cancel()
{
    document.forms['preset'].action='staffmainpage.php';
