...
if($_SERVER['REQUEST_METHOD']=='POST')
{
        ...
        $Password= $_POST['password'];
         $sql="SELECT * FROM `committee_login` WHERE EMAIL='$Username' && PASSWORD='$Password'";
          $result=$mysql->query($sql);
          if ($result->num_rows > 0) 
          {
			while($row = $result->fetch_assoc()) {
				 $_SESSION['cmmtyname']= $row["COMMITTEE_NAME"];
					}
			if(isset($_POST['username'])){
				$url="pages/greplay.php";
				header('Location:' .$url);
				exit();
			}
          } 
	...
	<div class="d-flex justify-content-center h-100">
		<div class="card">
			<div class="card-header">
				<h3 align="center" style="color:white">COMMITTEE LOGIN!</h3>
			</div>
			<div class="card-body">
